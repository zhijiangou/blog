...
export default function PostsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'mood' | 'blog'>('all');

  const filteredPosts = posts.filter(post => {
    if (filter === 'all') return true;
    if (filter === 'mood') return post.isMood;
    return !post.isMood;
  });
    ...
    <main className="min-h-screen flex flex-col relative" style={{ backgroundImage: 'url(https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=peony%20flowers%20border%20frame%2C%20white%20center%20space%20for%20text%2C%20elegant%20watercolor%20style%2C%20pink%20peonies%2C%20empty%20middle%20area%2C%20decorative%20floral%20border&image_size=landscape_16_9)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundAttachment: 'fixed' }}>
      ...
      <header className="py-16 px-6 border-b border-pink-100 relative z-10 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-gray-500 text-lg">记录生活点滴</p>
        </div>
      </header>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 text-sm rounded-full border border-pink-200 ${
                filter === 'all' ? 'bg-pink-500 text-white' : 'bg-white text-gray-700 hover:bg-pink-50'
              } transition-colors`}
            >
              全部
            </button>
            <button
              ...
              className={`px-3 py-1 text-sm rounded-full border border-pink-200 ${
                filter === 'mood' ? 'bg-pink-500 text-white' : 'bg-white text-gray-700 hover:bg-pink-50'
              } transition-colors`}
            >
              心情
            </button>
            <button
              ...
              className={`px-3 py-1 text-sm rounded-full border border-pink-200 ${
                filter === 'blog' ? 'bg-pink-500 text-white' : 'bg-white text-gray-700 hover:bg-pink-50'
              } transition-colors`}
            >
              博客
            </button>
      ...
      <section className="py-12 px-6 flex-1 relative z-10">
        <div className="max-w-4xl mx-auto space-y-8">
          {filteredPosts.map(post => (
            <article key={post.id} className="border-b border-pink-100 pb-8">
              <Link href={`/posts/${post.id}`} className="group">
                ...
                {post.title && (
                  <h2 className="text-2xl font-semibold mb-3 group-hover:text-pink-500 transition-colors text-gray-800">
                    {post.title}
                  </h2>
                )}
